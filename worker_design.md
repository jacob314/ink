...
Debugging complex terminal layouts, z-indexes, clipping, and scrolling can be difficult when looking at raw ANSI escapes. The `@examples/viewer/**` and replay tools provide a way to record render states and step through them offline.
...
```tsx
// examples/viewer/record-test.tsx
...
```bash
npx tsx examples/viewer/viewer.ts test-replay.json
```
