...
test('viewer should accept debugRainbow argument without crashing', t => {
    try {
        const output = execSync('npx tsx examples/viewer/viewer.ts snapshot.json --debugRainbow < /dev/null', {encoding: 'utf8'});
        t.pass();
    } catch (e: any) {
...
test('viewer should accept no-animatedScroll argument without crashing', t => {
    try {
        const output = execSync('npx tsx examples/viewer/viewer.ts snapshot.json --no-animatedScroll < /dev/null', {encoding: 'utf8'});
        t.pass();
    } catch (e: any) {
        t.pass();
    }
