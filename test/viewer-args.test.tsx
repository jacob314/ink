...
test('viewer should accept debugRainbow argument without crashing', t => {
	try {
		const output = execSync(
			'npx tsx tools/viewer/viewer.ts test/replay/scroll-demo.json --debugRainbow < /dev/null',
			{encoding: 'utf8'},
		);
		t.pass();
	} catch {
...
test('viewer should accept no-animatedScroll argument without crashing', t => {
	try {
		const output = execSync(
			'npx tsx tools/viewer/viewer.ts test/replay/scroll-demo.json --no-animatedScroll < /dev/null',
			{encoding: 'utf8'},
		);
		t.pass();
	} catch {
