...
test('TerminalBufferWorker avoids rerendering stuck headers during scroll', async t => {
	...
	t.is(term.buffer.active.getLine(1)?.translateToString(true).trim(), 'Content 2');
...
test('TerminalBufferWorker switches to stuckLines when stuck', async t => {
	...
	worker.update({id: 'root', children: []}, [
		{
			...
			lines: {
				updates: [{start: 0, end: 5, data: serializer.serialize([...naturalLines, ...naturalLines])}],
				totalLength: 5,
			},
		},
	]);
	...
	t.is(term.buffer.active.getLine(1)?.translateToString(true).trim(), 'NATURAL');
...
test('TerminalBufferWorker handles stuck headers with more lines than natural', async t => {
	...
	const stuckLines = [createStyledLine('STUCK L1'), createStyledLine('STUCK L2')]; // 2 lines
	...
	worker.update({id: 'root', children: []}, [
		{
			...
			lines: {
				updates: [{start: 0, end: 5, data: serializer.serialize([
	...
	t.is(term.buffer.active.getLine(0)?.translateToString(true).trim(), 'STUCK L1');
	t.is(term.buffer.active.getLine(1)?.translateToString(true).trim(), 'STUCK L2');
