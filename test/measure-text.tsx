...
test('handle tabs by expanding to 4 spaces', t => {
	...
	t.is(chars.length, 6);
	t.is(chars[0]?.getValue(), 'a');
	t.is(chars[1]?.getValue(), ' ');
	t.is(chars[2]?.getValue(), ' ');
	t.is(chars[3]?.getValue(), ' ');
	t.is(chars[4]?.getValue(), ' ');
	t.is(chars[5]?.getValue(), 'b');
});
