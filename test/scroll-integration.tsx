...
const writeToTerm = (term: any, data: string): Promise<void> =>
	new Promise(resolve => {
		term.write(data, () => {
			resolve();
		});
...
test('scroll integration - verify repaint efficiency', async t => {
	...
	const {unmount} = render(
		<ScrollableContent columns={columns} rows={termRows} itemCount={50} useStatic={false} />,
		{
			stdout,
			stdin,
			debugRainbow: true,
	...
	const getFullContent = () => {
		return Array.from({length: termRows}, (_, i) => 
	...
	const getScrollHeightFromTerm = () => {
		for (let i = 0; i < termRows; i++) {
			...
			const match = /Inner\s*scrollable\s*size:\s*\d+\s*x\s*(\d+)/i.exec(line || '');
	...
	for (let i = 0; i < termRows; i++) {
		...
		if (line && line.translateToString(true).trim()) {
			if (scrolledBgs[i] !== initialBgs[i]) {
				repaintedIndices.push(i);
			}
		}
	}

	console.log(`Total repainted non-empty lines: ${repaintedIndices.length} / ${termRows}`);
	...
	t.true(footerRepainted.length <= 2, 'Footer should not be repainted excessively during scroll');
	t.true(repaintedIndices.length < termRows / 2, 'Should not repaint the whole screen for a 1-line scroll');
	...
	console.log('Final content after scroll up:', finalContent.replace(/\s+/g, ' '));
	t.true(finalContent.includes('ScrollTop: 0'), 'Should be back at ScrollTop 0');
	...
	t.true(bottomContent.includes('ScrollTop: 710'), 'Should have scrolled to the bottom area');
